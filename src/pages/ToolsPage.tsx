          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="mb-8">
            <div className="flex items-center gap-3 mb-1">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-500/[0.08] border border-slate-500/15">
                <Wrench className="h-4 w-4 text-[#B6BBC6]/70" />
              </div>
              <h1 className="text-2xl font-semibold text-white tracking-tight">Tools</h1>
            </div>
            <p className="text-[13px] text-[#858B99] ml-11">All AI tools and generators in one place</p>
          </motion.div>

          {/* Search */}
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="mb-8">
            <div className="relative">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#858B99]" />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search tools..."
                className="w-full h-11 pl-11 pr-4 rounded-xl bg-white/[0.02] border border-white/[0.04] text-[13px] text-slate-300 placeholder:text-[#858B99] focus:outline-none focus:border-[#8B5CF6]/20 focus:bg-white/[0.03] transition-all"
              />
            </div>
          </motion.div>
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {tool.isNew && (
                            <span className="text-[8px] font-semibold px-1.5 py-0.5 rounded bg-[#8B5CF6]/[0.1] text-[#8B5CF6] uppercase tracking-wider">New</span>
                          )}
                          {tool.path && (
                            <ExternalLink className="w-3 h-3 text-[#858B99] opacity-0 group-hover:opacity-100 transition-opacity" />
                          )}
                        </div>
                        <p className="text-[11px] text-[#858B99] mt-0.5">{tool.desc}</p>
                      </div>
                      {tool.path && (
                        <ArrowRight className="w-4 h-4 text-[#858B99] group-hover:text-[#A78BFA] transition-colors shrink-0" />
                      )}
                    </motion.button>
                  ))}
                </div>
