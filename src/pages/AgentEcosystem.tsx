    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white relative">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-6 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-[13px] text-slate-500">12 production agents. Deploy any with one click.</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="mb-6 flex items-center gap-4 py-3 px-4 rounded-2xl border border-white/[0.03] bg-white/[0.01] overflow-x-auto">
          ...
          <span className="text-[11px] text-slate-600 shrink-0">{AGENTS.reduce((a, g) => a + g.conversations, 0).toLocaleString()} conversations</span>
          <span className="text-[11px] text-slate-600 shrink-0">{AGENTS.filter((a) => a.status === 'live').length} agents live</span>
          <span className="text-[11px] text-slate-600 shrink-0">{AGENTS.filter((a) => a.status === 'beta').length} in beta</span>
        </motion.div>
          ...
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[11px] font-medium transition-all shrink-0 ${
                activeCategory === cat ? 'bg-white/[0.06] text-white' : 'text-slate-600 hover:text-slate-400'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
          {filtered.map((agent, i) => (
            ...
            >
              ...
              <p className="text-[11px] text-slate-600 leading-relaxed mb-3 line-clamp-2">{agent.description}</p>
          ...
          >
            ...
            >
              ...
              <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-white/[0.04]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-600">{selectedAgent.description}</p>
                  </div>
                </div>
                <button onClick={() => setSelectedAgent(null)} className="h-7 w-7 flex items-center justify-center rounded-lg text-slate-600 hover:text-white hover:bg-white/[0.05] transition-all">
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 scrollbar-thin">
                ...
                {/* Features */}
                <div>
                  <p className="text-[11px] font-semibold text-slate-600 uppercase tracking-wider mb-2">Capabilities</p>
                  <div className="flex flex-wrap gap-1.5">
                    {selectedAgent.features.map((f) => (
                      <span key={f} className="text-[10px] px-2 py-1 rounded-lg bg-white/[0.02] border border-white/[0.03] text-slate-500">{f}</span>
                    ))}
                  </div>
                </div>
                ...
                {/* Quick examples */}
                <div>
                  <p className="text-[11px] font-semibold text-slate-600 uppercase tracking-wider mb-2">Try these</p>
                ...
                <div className="space-y-3">
                  {selectedAgent.inputs.map((input, i) => (
                    <div key={i}>
                      <label className="text-[11px] text-slate-500 mb-1.5 block">{input.label}</label>
                      {input.type === 'textarea' ? (
                        <textarea
                          value={formValues[i] || ''}
