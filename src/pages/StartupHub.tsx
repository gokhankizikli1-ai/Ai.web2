...
const COLOR_MAP: Record<string, { bg: string; border: string; icon: string }> = {
  ...
  cyan:   { bg: 'bg-[#3B82F6]/[0.05]',   border: 'border-[#3B82F6]/20',   icon: 'text-[#60A5FA]' },
  violet: { bg: 'bg-[#3B82F6]/[0.05]', border: 'border-[#3B82F6]/10', icon: 'text-[#60A5FA]' },
  emerald:{ bg: 'bg-[#3B82F6]/[0.05]',border: 'border-[#3B82F6]/10',icon: 'text-[#60A5FA]' },
  rose:   { bg: 'bg-[#3B82F6]/[0.05]',   border: 'border-[#3B82F6]/10',   icon: 'text-[#60A5FA]' },
  blue:   { bg: 'bg-[#3B82F6]/[0.05]',   border: 'border-[#3B82F6]/10',   icon: 'text-[#60A5FA]' },
  indigo: { bg: 'bg-[#3B82F6]/[0.05]', border: 'border-[#3B82F6]/10', icon: 'text-[#60A5FA]' },
  pink:   { bg: 'bg-[#3B82F6]/[0.05]',   border: 'border-[#3B82F6]/10',   icon: 'text-[#60A5FA]' },
  teal:   { bg: 'bg-[#3B82F6]/[0.05]',   border: 'border-[#3B82F6]/10',   icon: 'text-[#60A5FA]' },
  orange: { bg: 'bg-[#3B82F6]/[0.05]', border: 'border-[#3B82F6]/10', icon: 'text-[#60A5FA]' },
};
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
            <div className="flex items-center gap-3 mb-4">
              <button onClick={() => navigate('/workspace')} className="flex items-center gap-1.5 text-[11px] text-[#94A3B8] hover:text-slate-300 transition-colors">
                <ArrowLeft className="h-3 w-3" /> Workspace
              </button>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#3B82F6]/[0.1] border border-[#3B82F6]/25">
              ...
              <div>
                ...
                <p className="text-[12px] text-[#94A3B8]">Find where the market is angry before you build.</p>
              </div>
            </div>
          </motion.div>
          ...
          <details className="mt-8 group">
            <summary className="flex items-center gap-2 cursor-pointer select-none text-[12px] text-[#94A3B8] hover:text-slate-300 transition-colors list-none">
              ...
              <AnimatePresence mode="wait">
                {!selectedTool ? (
                      ...
                      {TOOLS.map((tool, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <p className="text-[10px] text-[#94A3B8] leading-relaxed">{tool.desc}</p>
                            <div className="flex items-center gap-1 mt-2 text-[#94A3B8] group-hover/tool:text-[#94A3B8] transition-colors">
                  ...
                  <motion.div key="tool" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                    <button onClick={() => setSelectedTool(null)} className="flex items-center gap-1 text-[12px] text-[#94A3B8] hover:text-white transition-colors mb-4">
                      ...
                      <>
                        <div className="flex items-center gap-3 mb-5">
                          ...
                          <div>
                            ...
                            <p className="text-[12px] text-[#94A3B8]">{activeTool.desc}</p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4 mb-5">
                          {activeTool.inputs.map((input, i) => (
                            <div key={i}>
                              <label className="block text-[12px] text-[#CBD5E1] mb-1.5">{input.label}</label>
                              {(input as { type?: string }).type === 'textarea' ? (
                                <textarea
                                  ...
                                  className="w-full rounded-xl bg-white/[0.015] border border-white/[0.04] p-3 text-[13px] text-white placeholder:text-[#94A3B8] focus:border-[#3B82F6]/30 focus:bg-white/[0.02] outline-none transition-all resize-none"
                                />
                              ) : (
                                <input
                                  type="text" value={formValues[i] || ''} onChange={(e) => setFormValues((p) => ({ ...p, [i]: e.target.value }))}
                                  placeholder={input.placeholder}
                                  className="w-full h-10 rounded-xl bg-white/[0.015] border border-white/[0.04] px-3 text-[13px] text-white placeholder:text-[#94A3B8] focus:border-[#3B82F6]/30 focus:bg-white/[0.02] outline-none transition-all"
                                />
                              )}
                            </div>
                          ))}
