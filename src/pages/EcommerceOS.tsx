...
const COLOR_MAP: Record<string, { bg: string; border: string; icon: string; glow: string }> = {
  ...
  rose:    { bg: 'bg-[#3B82F6]/[0.05]',    border: 'border-[#3B82F6]/10',    icon: 'text-[#60A5FA]',    glow: 'hover:shadow-[0_0_20px_-4px_rgba(59, 130, 246,0.08)]' },
  ...
  pink:    { bg: 'bg-[#3B82F6]/[0.05]',    border: 'border-[#3B82F6]/10',    icon: 'text-[#60A5FA]',    glow: 'hover:shadow-[0_0_20px_-4px_rgba(59, 130, 246,0.08)]' },
  teal:    { bg: 'bg-[#3B82F6]/[0.05]',    border: 'border-[#3B82F6]/10',    icon: 'text-[#60A5FA]',    glow: 'hover:shadow-[0_0_20px_-4px_rgba(59, 130, 246,0.08)]' },
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#3B82F6]/[0.08] border border-[#3B82F6]/15">
                <ShoppingCart className="h-5 w-5 text-[#60A5FA]/70" />
              </div>
              <div>
                <h1 className="text-2xl font-semibold text-white tracking-tight">Ecommerce OS</h1>
                <p className="text-[12px] text-[#94A3B8]">From research to revenue</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 mt-4">
              {[
                { icon: DollarSign, label: 'Revenue', value: '$24,680', color: 'text-[#60A5FA]' },
                { icon: ShoppingCart, label: 'Orders', value: '1,247', color: 'text-[#60A5FA]' },
                { icon: Users, label: 'Visitors', value: '8.4K', color: 'text-[#60A5FA]' },
                { icon: TrendingUp, label: 'Conversion', value: '3.2%', color: 'text-[#60A5FA]' },
                { icon: Award, label: 'AOV', value: '$47.20', color: 'text-[#60A5FA]' },
              ].map((s, i) => (
            ...
            {!selected ? (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          ...
                          {tool.badge && (
                            <span className={`text-[9px] font-semibold px-1.5 py-[2px] rounded ${tool.badge === 'Popular' ? 'bg-[#3B82F6]/[0.08] text-[#60A5FA]/70' : 'bg-[#3B82F6]/[0.08] text-[#60A5FA]/70'}`}>
                        ...
                        <p className="text-[11px] text-[#94A3B8] leading-relaxed">{tool.desc}</p>
                        <div className="flex items-center gap-1 mt-2 text-[#94A3B8] group-hover:text-[#94A3B8] transition-colors">
              ...
              <motion.div key="tool" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
                <div className="flex items-center gap-3 mb-6">
                  <button onClick={() => setSelected(null)} className="flex items-center gap-1 text-[12px] text-[#94A3B8] hover:text-white transition-colors">
                        ...
                        <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
                          ...
                          {(input as any).type === 'textarea' ? (
                            <textarea value={formValues[i] || ''} onChange={(e) => setFormValues((p) => ({ ...p, [i]: e.target.value }))} placeholder={input.placeholder} rows={4}
                              className="w-full rounded-xl bg-white/[0.015] border border-white/[0.04] p-3 text-[13px] text-white placeholder:text-[#94A3B8] focus:border-[#3B82F6]/20 focus:bg-white/[0.02] outline-none transition-all resize-none" />
                          ) : (
                            <input type="text" value={formValues[i] || ''} onChange={(e) => setFormValues((p) => ({ ...p, [i]: e.target.value }))} placeholder={input.placeholder}
                              className="w-full h-10 rounded-xl bg-white/[0.015] border border-white/[0.04] px-3 text-[13px] text-white placeholder:text-[#94A3B8] focus:border-[#3B82F6]/20 focus:bg-white/[0.02] outline-none transition-all" />
                          )}
                        </motion.div>
                      ))}
                    </div>
