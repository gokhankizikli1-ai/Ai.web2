        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
          ...
          <motion.div {...fadeUp(0)} className="mb-8">
            <div className="flex items-center gap-3 mb-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#7EA6BF]/[0.1] border border-[#7EA6BF]/15">
                <Flame className="h-4 w-4 text-[#9CBBD1]" />
              </div>
              <h1 className="text-2xl font-semibold text-white tracking-tight">Viral Content Engine</h1>
            </div>
            <p className="text-[13px] text-[#7F8FA3] ml-11">Generate TikTok scripts, YouTube ideas, Instagram carousels, X threads, and more</p>
          </motion.div>
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[11px] font-medium transition-all whitespace-nowrap ${
                    activeType === t.id
                      ...
                      : 'text-[#7F8FA3] hover:text-slate-300 bg-white/[0.02] border border-white/[0.03]'
                  }`}
                >
          ...
          <motion.div {...fadeUp(0.08)} className="mb-6">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 h-11 px-4 rounded-xl bg-white/[0.02] border border-white/[0.04] text-[14px] text-slate-300 placeholder:text-[#7F8FA3] focus:outline-none focus:border-[#7EA6BF]/20 focus:bg-white/[0.03] transition-all"
              ...
              <motion.button
                ...
                className="h-11 px-5 rounded-xl bg-[#7EA6BF]/[0.1] border border-[#7EA6BF]/15 text-[#9CBBD1] font-medium text-[13px] hover:bg-[#7EA6BF]/[0.15] transition-colors disabled:opacity-40 flex items-center gap-2"
              >
          ...
          {output && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
              {output.map((line, i) => (
                ...
                >
                  <span className="text-[10px] text-[#7F8FA3] font-mono mt-0.5 shrink-0">{String(i + 1).padStart(2, '0')}</span>
          ...
          {!output && !generating && (
            <motion.div {...fadeUp(0.1)} className="text-center py-16">
              <Flame className="w-12 h-12 text-[#7F8FA3] mx-auto mb-4" />
              <h3 className="text-sm font-medium text-white mb-1">Select a content type and enter a topic</h3>
              <p className="text-[12px] text-[#7F8FA3]">AI will generate viral content optimized for your platform</p>
            </motion.div>
          )}
        </div>
      </div>
