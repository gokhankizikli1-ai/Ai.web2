...
const WORKSPACES = [
  {
    ...
    bg: 'bg-[#8B5CF6]/[0.06]', border: 'border-[#8B5CF6]/10', iconColor: 'text-[#A78BFA]',
    glow: 'hover:shadow-[0_0_24px_-6px_rgba(139, 92, 246,0.1)]',
...
const RECENT_ITEMS = [
  { title: 'NVDA Q3 Earnings Analysis', workspace: 'Deep Research', time: '2m ago', icon: Brain, color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
  { title: 'Landing Page Copy — Acme AI', workspace: 'Startup Hub', time: '1h ago', icon: Rocket, color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
  { title: 'Shopify Ad Campaign #3', workspace: 'Ecommerce OS', time: '3h ago', icon: ShoppingBag, color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
  { title: 'AAPL Technical Analysis', workspace: 'Trading Desk', time: '5h ago', icon: TrendingUp, color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
  { title: 'React Component Review', workspace: 'Code Studio', time: 'Yesterday', icon: Code2, color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
];
...
function WorkspaceCard({ ws }: { ws: typeof WORKSPACES[0] }) {
  ...
  return (
    ...
    >
      {/* Pinned badge */}
      {ws.pinned && (
        <div className="absolute top-3 right-3 flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-[#8B5CF6]/[0.06] border border-[#8B5CF6]/10">
          <Pin className="w-2.5 h-2.5 text-[#A78BFA]/60" />
          <span className="text-[9px] text-[#A78BFA]/60">Pinned</span>
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-[11px] text-[#858B99] mt-0.5">{ws.description}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-3">
        {Object.entries(ws.stats).map(([key, val]) => (
          <span key={key} className="text-[10px] text-[#B6BBC6] px-2 py-1 rounded-lg bg-white/[0.02] border border-white/[0.03] flex items-center gap-1">
      ...
      {/* Hover action */}
      <div className="flex items-center gap-1 text-[11px] text-[#858B99] group-hover:text-[#A78BFA]/70 transition-colors">
...
function StatsBar() {
  const stats = [
    { icon: Zap, label: 'Active', value: '7', color: 'text-[#A78BFA]' },
    { icon: Activity, label: 'Running', value: '3', color: 'text-[#4ADE80]' },
    { icon: Star, label: 'Pinned', value: '3', color: 'text-[#A78BFA]' },
    { icon: Clock, label: 'This Week', value: '42', color: 'text-[#A78BFA]' },
  ];
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#8B5CF6]/[0.08] border border-[#8B5CF6]/15">
                  <LayoutGrid className="h-5 w-5 text-[#A78BFA]/70" />
                </div>
                <div>
                  <h1 className="text-2xl font-semibold text-white tracking-tight">Workspace</h1>
                  <p className="text-[12px] text-[#858B99]">Manage your AI workspaces</p>
                </div>
              </div>
              <button className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#8B5CF6]/[0.08] border border-[#8B5CF6]/15 text-[12px] text-[#A78BFA] hover:bg-[#8B5CF6]/[0.12] transition-colors">
          ...
          <motion.section {...fadeUp(0.05)} className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              <Star className="w-3.5 h-3.5 text-[#A78BFA]/60" />
          ...
          <motion.section {...fadeUp(0.1)} className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              <LayoutGrid className="w-3.5 h-3.5 text-[#858B99]" />
          ...
          <motion.section {...fadeUp(0.15)}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Clock className="w-3.5 h-3.5 text-[#858B99]" />
                <h2 className="text-[13px] font-semibold text-white">Recent Activity</h2>
              </div>
              <span className="text-[10px] text-[#858B99]">{RECENT_ITEMS.length} items</span>
            </div>
              ...
              {RECENT_ITEMS.map((item, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-[13px] font-medium text-white group-hover:text-[#A78BFA] transition-colors truncate">{item.title}</h3>
                    <p className="text-[11px] text-[#858B99]">{item.workspace}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[11px] text-[#858B99]">{item.time}</span>
                    <ChevronRight className="w-3.5 h-3.5 text-[#B6BBC6] group-hover:text-[#A78BFA]/60 transition-colors" />
                  </div>
                </motion.div>
              ))}
            </div>
