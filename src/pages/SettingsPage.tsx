          ...
          {LANGUAGES.map((l) => (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 text-left text-[12px] transition-all ${
                lang === l.code ? 'bg-white/[0.06] text-white' : 'text-[#94A3B8] hover:text-slate-300 hover:bg-white/[0.02]'
              }`}
            >
              <span className="text-[10px] font-medium text-[#94A3B8] w-5">{l.flag}</span>
              {l.label}
            </button>
          ))}
        </div>
      ...
      <main className="pt-24 pb-12 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-8">
            <Link to="/chat">
              <Button variant="ghost" size="icon" className="h-8 w-8 text-[#94A3B8] hover:text-white hover:bg-white/[0.05] rounded-lg">
                <ArrowLeft className="h-4 w-4" />
              </Button>
            </Link>
            <div>
              <h1 className="text-xl font-semibold text-white">Settings</h1>
              <p className="text-[12px] text-[#94A3B8]">Customize your KorvixAI workspace</p>
            </div>
          </div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 rounded-lg px-3 py-2 text-left text-[13px] transition-all duration-200 ${
                        isActive
                          ...
                          : 'text-[#94A3B8] hover:text-slate-300 hover:bg-white/[0.02] border border-transparent'
                      }`}
                    >
                      <section.icon className={`h-4 w-4 ${isActive ? 'text-[#3B82F6]' : 'text-[#94A3B8]'}`} />
                      {section.label}
                    </button>
                  );
                })}
            ...
            >
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] overflow-hidden">
                <div className="flex items-center gap-2 px-5 py-4 border-b border-white/[0.04]">
                  <currentSection.icon className="h-4 w-4 text-[#3B82F6]/60" />
                    ...
                    <div key={setting.id} className="flex items-center justify-between px-5 py-4 hover:bg-white/[0.01] transition-colors">
                      ...
                      {setting.type === 'text' && (
                        <input
                          ...
                          className="bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-1.5 text-[12px] text-white w-48 outline-none focus:border-[#3B82F6]/30 transition-colors"
                        />
                      )}
                      ...
                      {setting.type === 'select' && (
                        <select
                          defaultValue={String(setting.value)}
                          className="bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-1.5 text-[12px] text-white outline-none focus:border-[#3B82F6]/30 transition-colors appearance-none cursor-pointer"
                        >
              ...
              {activeSection === 'privacy' && (
                <div className="mt-6 rounded-2xl border border-[#F87171]/[0.08] bg-[#F87171]/[0.02] p-5">
                  ...
                  <p className="text-[12px] text-[#94A3B8] mb-4">These actions cannot be undone.</p>
                  <Button
                    variant="ghost"
