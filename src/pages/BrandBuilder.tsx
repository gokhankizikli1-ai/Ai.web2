...
const COLOR_PALETTE = [
  ...
  { name: 'Electric Cyan', hex: '#22D3EE', role: 'Accent' },
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
          ...
          <motion.div {...fadeUp(0)} className="mb-8">
            <div className="flex items-center gap-3 mb-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-pink-500/[0.1] border border-pink-500/15">
          ...
          <motion.div {...fadeUp(0.05)} className="mb-6">
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-3 rounded-xl bg-white/[0.02] border border-white/[0.04] text-[14px] text-slate-300 placeholder:text-slate-600 focus:outline-none focus:border-pink-500/20 focus:bg-white/[0.03] transition-all resize-none"
            />
            <div className="flex justify-end mt-2">
              <motion.button
                ...
                className="h-10 px-5 rounded-xl bg-pink-500/[0.1] border border-pink-500/15 text-pink-400 font-medium text-[13px] hover:bg-pink-500/[0.15] transition-colors disabled:opacity-40 flex items-center gap-2"
              >
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              <div className="p-5 rounded-2xl border border-white/[0.03] bg-white/[0.01]">
                <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-pink-400" /> Brand Name Ideas
                </h3>
                <div className="flex gap-2 flex-wrap">
                  {BRAND_NAMES.map((name) => (
                    ...
                    >
                      ...
                      {copied === `name-${name}` ? <CheckCircle2 className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3 text-slate-600 group-hover:text-slate-400" />}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="p-5 rounded-2xl border border-white/[0.03] bg-white/[0.01]">
                <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
                  <PenTool className="w-4 h-4 text-pink-400" /> Slogan Options
                </h3>
                  ...
                  {SLOGANS.map((slogan, i) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02]">
                      ...
                      <button onClick={() => copy(slogan, `slogan-${i}`)}>
                        {copied === `slogan-${i}` ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-slate-600 hover:text-slate-400" />}
                      </button>
                    </div>
                  ))}
                </div>
              ...
              <div className="p-5 rounded-2xl border border-white/[0.03] bg-white/[0.01]">
                <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
                  <Palette className="w-4 h-4 text-pink-400" /> Color Palette
                </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-5 rounded-2xl border border-white/[0.03] bg-white/[0.01]">
                  <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
                    <Target className="w-4 h-4 text-pink-400" /> Tone of Voice
                  </h3>
                ...
                <div className="p-5 rounded-2xl border border-white/[0.03] bg-white/[0.01]">
                  <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
                    <Users className="w-4 h-4 text-pink-400" /> Target Audience
                  </h3>
                  <div className="space-y-1.5">
                    {AUDIENCES.map((a, i) => (
