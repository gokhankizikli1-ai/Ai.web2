...
export function buildWebBuildRequest(
  ...
  if (opts?.revise) {
    ...
    lines.push(
      ...
      '',
      'MANDATORY: the two agents are PREPARATION, not the deliverable. You MUST NOT stop',
      'after the Design Direction. Every build MUST output ALL six sections below —',
      'especially ## Page Sections, ## Generated Copy and ## Frontend Code. A reply that',
      'ends after the agent/design analysis is an INCOMPLETE build and is not acceptable.',
      '',
