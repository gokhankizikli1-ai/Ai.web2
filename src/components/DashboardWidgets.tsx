...
export function SuggestionsWidget() {
  ...
  const suggestions = [
    { icon: Brain, text: 'Analyze Tesla Q3 earnings report', path: '/chat', color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
    { icon: Rocket, text: 'Validate your SaaS startup idea', path: '/startup', color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
    { icon: ShoppingBag, text: 'Generate TikTok hooks for your product', path: '/tools/viral-content', color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
    { icon: FileText, text: 'Build a landing page for your brand', path: '/tools/website-builder', color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
  ];
      ...
      <div className="space-y-2">
        {suggestions.map((s, i) => (
          ...
          >
            ...
            <span className="text-[12px] text-[#B6BBC6] group-hover:text-slate-200 transition-colors flex-1">{s.text}</span>
            <ArrowRight className="w-3 h-3 text-[#858B99] group-hover:text-[#8B5CF6] transition-colors" />
          </motion.button>
        ))}
      </div>
    </WidgetCard>
...
export function StatsRowWidget() {
  const stats = [
    { label: 'Conversations', value: '24', icon: MessageSquare, color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
    { label: 'Agents Active', value: '3', icon: Bot, color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
    { label: 'Signals Today', value: '12', icon: TrendingUp, color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
    { label: 'Research Tasks', value: '5', icon: Brain, color: 'text-[#A78BFA]', bg: 'bg-[#8B5CF6]/[0.06]' },
  ];
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {stats.map((s, i) => (
        ...
        >
          ...
          <p className="text-[11px] text-[#858B99] mt-0.5">{s.label}</p>
        </motion.div>
      ))}
    </div>
  );
...
export function QuickActionsWidget() {
  ...
  const COLOR_MAP: Record<string, string> = {
    cyan: 'hover:shadow-[0_0_20px_-4px_rgba(139, 92, 246,0.12)] hover:border-[#A78BFA]/20',
    orange: 'hover:shadow-[0_0_20px_-4px_rgba(139, 92, 246,0.12)] hover:border-[#A78BFA]/20',
    emerald: 'hover:shadow-[0_0_20px_-4px_rgba(139, 92, 246,0.12)] hover:border-[#4ADE80]/20',
    violet: 'hover:shadow-[0_0_20px_-4px_rgba(139, 92, 246,0.12)] hover:border-[#A78BFA]/20',
    indigo: 'hover:shadow-[0_0_20px_-4px_rgba(139, 92, 246,0.12)] hover:border-[#A78BFA]/20',
    blue: 'hover:shadow-[0_0_20px_-4px_rgba(139, 92, 246,0.12)] hover:border-[#A78BFA]/20',
  };
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {actions.map((a) => (
          ...
          >
            ...
            <span className="text-[10px] text-[#858B99]">{a.desc}</span>
          </motion.button>
        ))}
      </div>
    </WidgetCard>
...
export function ActivityTimelineWidget() {
  const items = [
    { action: 'Deep Research completed', detail: 'NVDA Q3 Earnings Analysis', time: '2m ago', icon: Brain, color: 'text-[#A78BFA]' },
    { action: 'Trading signal detected', detail: 'AAPL Long — 87% confidence', time: '15m ago', icon: TrendingUp, color: 'text-[#4ADE80]' },
    { action: 'New chat started', detail: 'System Architecture Discussion', time: '32m ago', icon: MessageSquare, color: 'text-[#A78BFA]' },
    { action: 'Agent task finished', detail: 'Market Scanner — 2 signals', time: '1h ago', icon: Bot, color: 'text-[#A78BFA]' },
    { action: 'Landing page generated', detail: 'Acme AI — conversion optimized', time: '2h ago', icon: FileText, color: 'text-[#A78BFA]' },
  ];
        ...
        {items.map((item, i) => (
          <div key={i} className="flex items-start gap-3 py-2.5 border-b border-white/[0.02] last:border-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-[#858B99] truncate">{item.detail}</p>
            </div>
            <span className="text-[10px] text-[#858B99] shrink-0">{item.time}</span>
          </div>
        ))}
      </div>
    </WidgetCard>
...
export function ActiveAgentsWidget() {
  const agents = [
    ...
    { name: 'Code Reviewer', status: 'active', color: 'bg-[#A78BFA]', icon: Code2 },
    { name: 'Research AI', status: 'idle', color: 'bg-[#A78BFA]', icon: Brain },
  ];
...
export function WorkspaceActivityWidget() {
  ...
  return (
    <WidgetCard title="Workspace Activity" icon={<BarChart3 className="w-3.5 h-3.5" />} delay={0.2}>
      ...
      <p className="text-[11px] text-[#858B99] mt-2">Most active: <span className="text-[#A78BFA]">Startup Hub</span></p>
    </WidgetCard>
  );
}
...
export function TrendingStartupsWidget() {
  const startups = [
    { name: 'AI Contract Intelligence', category: 'LegalTech', score: 94, trend: '+12%', color: '#A78BFA' },
    { name: 'Carbon Intelligence Platform', category: 'ClimateTech', score: 89, trend: '+8%', color: '#4ADE80' },
    { name: 'Cross-Chain Liquidity Router', category: 'DeFi', score: 86, trend: '+15%', color: '#A78BFA' },
  ];
          ...
          <div key={i} className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-[10px] text-[#858B99]">{s.category}</span>
        ...
        {products.map((p, i) => (
          <div key={i} className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.03]">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button className="text-[10px] text-[#A78BFA] hover:text-[#A78BFA] transition-colors">Analyze</button>
            </div>
            <div className="flex items-center gap-3 text-[10px] text-[#858B99]">
...
export function MarketSnapshotWidget() {
  return (
    <WidgetCard title="Market Snapshot" icon={<TrendingUp className="w-3.5 h-3.5 text-[#4ADE80]" />} delay={0.35}>
      ...
      <p className="text-[11px] text-[#858B99]">AAPL — Trading data from simulated feed</p>
    </WidgetCard>
  );
}
      ...
      <div className="p-5 pt-0">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 h-10 px-4 rounded-xl bg-white/[0.02] border border-white/[0.04] text-[13px] text-slate-300 placeholder:text-[#858B99] focus:outline-none focus:border-[#A78BFA]/20 focus:bg-white/[0.03] transition-all"
          />
          <button className="h-10 px-4 rounded-xl bg-[#A78BFA]/[0.1] border border-[#A78BFA]/15 text-[#A78BFA] hover:bg-[#A78BFA]/[0.15] transition-colors">
            <Sparkles className="w-4 h-4" />
          </button>
        </div>
        <div className="flex gap-2 mt-3 flex-wrap">
          {['Landing page', 'SaaS idea', 'Shopify product', 'Brand kit'].map((chip) => (
            <span key={chip} className="px-2.5 py-1 rounded-lg bg-white/[0.02] text-[10px] text-[#B6BBC6] border border-[#8B5CF6]/25 hover:border-[#8B5CF6]/40 hover:bg-[#8B5CF6]/[0.06] cursor-pointer transition-colors">
              {chip}
            </span>
          ))}
        </div>
      ...
      <div className="h-28">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} barSize={20}>
            ...
            <YAxis hide />
            <Bar dataKey="conversations" fill="#A78BFA" fillOpacity={0.4} radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      <p className="text-[11px] text-[#858B99] mt-1">Peak: Thursday with 24 conversations</p>
    </WidgetCard>
  );
}
...
export function RecentGenerationsWidget() {
  const generations = [
    { type: 'Landing Page', preview: 'Hero section with CTA for AI analytics SaaS...', time: '5m ago', color: 'text-[#A78BFA]', bg: 'bg-[#A78BFA]/[0.06]' },
    { type: 'TikTok Script', preview: 'Hook: "This $19 gadget saved me $400..."', time: '12m ago', color: 'text-[#F87171]', bg: 'bg-[#F87171]/[0.06]' },
    { type: 'Market Research', preview: 'TAM: $4.2B, SAM: $680M, SOM: $45M...', time: '1h ago', color: 'text-[#A78BFA]', bg: 'bg-[#A78BFA]/[0.06]' },
    { type: 'Brand Kit', preview: 'Name: "Nexora", Tagline: "Intelligence..."', time: '2h ago', color: 'text-[#A78BFA]', bg: 'bg-[#A78BFA]/[0.06]' },
  ];

  return (
    <WidgetCard title="AI Generations" icon={<Sparkles className="w-3.5 h-3.5 text-[#A78BFA]" />} delay={0.5}>
      <div className="space-y-2">
        {generations.map((g, i) => (
          <div key={i} className="flex items-start gap-2.5 p-2.5 rounded-xl hover:bg-white/[0.02] transition-colors cursor-pointer group">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[11px] text-[#B6BBC6] truncate group-hover:text-slate-300 transition-colors">{g.preview}</p>
            </div>
            <span className="text-[10px] text-[#858B99] shrink-0">{g.time}</span>
          </div>
        ))}
      </div>
    </WidgetCard>
