      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-3 mt-2">
          ...
          <AnimatedNumber
            ...
            format={(n) => `${Math.round(n)}% conf`}
            className="text-[11px] text-slate-500"
          />
          <span className="text-[11px] text-slate-600 capitalize">{signal.volatility} vol</span>
          <span className="ml-auto"><HeatBar pct={signal.analytics?.volatilityPct ?? undefined} tier={signal.volatility} /></span>
        </div>
...
function MarketRegimeBanner({ r }: { r: MarketRegime }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3 }}
      className={`relative overflow-hidden rounded-xl border ${g} px-4 py-2.5`}
    >
      {/* subtle market-pulse glow (decorative; reflects an active regime) */}
      <motion.div
        aria-hidden
        className="pointer-events-none absolute -inset-y-6 -left-10 w-24 rounded-full bg-current opacity-[0.06] blur-2xl"
        animate={{ x: ['-20%', '160%'] }}
      ...
      <p className="relative text-[11px] text-slate-400 mt-1 leading-relaxed">{r.note}</p>
    </motion.div>
  );
}
        ...
        <div className="flex items-center justify-between gap-3 mb-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="hidden sm:flex items-center text-[10px] text-slate-600">
              ...
              <Pulse trigger={activeApi.lastUpdated ?? ''}>
