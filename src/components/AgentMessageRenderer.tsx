...
function iconFor(heading: string): { Icon: typeof ListTree; tone: string } {
  ...
  if (h.startsWith('plan'))                 return { Icon: Lightbulb, tone: 'text-[#A78BFA]/70' };
  if (h.startsWith('intent'))               return { Icon: Wand2,     tone: 'text-[#A78BFA]/70' };
  if (h.startsWith('design'))               return { Icon: Sparkles,  tone: 'text-[#A78BFA]/70' };
  if (h.startsWith('component'))            return { Icon: ListTree,  tone: 'text-[#A78BFA]/70' };
  if (h.startsWith('file'))                 return { Icon: FolderTree,tone: 'text-[#A78BFA]/70' };
  if (h.startsWith('implementation'))       return { Icon: Layers,    tone: 'text-[#A78BFA]/70' };
  if (h.startsWith('code'))                 return { Icon: FileCode2, tone: 'text-[#A78BFA]/70' };
  if (h.startsWith('next'))                 return { Icon: ListChecks,tone: 'text-[#A78BFA]/70' };
  if (h.startsWith('recommendation'))       return { Icon: Lightbulb, tone: 'text-[#A78BFA]/70' };
...
const mdComponents = {
  ...
  code: ({ inline, className, children, ...rest }: {
    ...
    if (inline) {
      return (
        <code className="px-1 py-0.5 rounded text-[11px] font-mono text-[#A78BFA]/90 bg-[#8B5CF6]/[0.06] border border-[#8B5CF6]/[0.08]" {...rest}>
          {children}
        </code>
      );
    }
  ...
  blockquote: ({ children }: { children?: React.ReactNode }) => (
    <blockquote className="border-l-2 border-[#8B5CF6]/30 pl-2.5 my-1.5 text-[11.5px] text-white/55 italic">
      {children}
    </blockquote>
  ),
  ...
  a: ({ href, children }: { href?: string; children?: React.ReactNode }) => (
    <a href={href} target="_blank" rel="noreferrer noopener"
       className="text-[#A78BFA]/80 hover:text-[#A78BFA] underline decoration-[#8B5CF6]/30 underline-offset-2">
      {children}
    </a>
  ),
