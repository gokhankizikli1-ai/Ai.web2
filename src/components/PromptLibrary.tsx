      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          <div className="flex items-center gap-2">
            <Bookmark className="h-4 w-4 text-[#7EA6BF]" />
        ...
        <div className="px-5 pb-3">
          <div className="flex items-center gap-2 rounded-xl bg-white/[0.03] border border-white/[0.06] px-3 py-2 focus-within:border-[#7EA6BF]/20 transition-colors">
            <Search className="h-3.5 w-3.5 text-[#7F8FA3]" />
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search prompts..."
              className="flex-1 bg-transparent text-[13px] text-white placeholder:text-[#7F8FA3] outline-none"
              autoFocus
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 px-5 pb-3 overflow-x-auto scrollbar-thin">
          <button
            ...
            className={`shrink-0 rounded-lg px-2.5 py-[3px] text-[11px] font-medium transition-all duration-200 ${
              activeCategory === 'All' ? 'bg-white/[0.07] text-white border border-white/[0.08]' : 'text-[#7F8FA3] hover:text-slate-300 hover:bg-white/[0.03]'
            }`}
          >
            All
          </button>
          {categories.map((cat) => (
            <button
              ...
              className={`shrink-0 rounded-lg px-2.5 py-[3px] text-[11px] font-medium transition-all duration-200 ${
                activeCategory === cat ? 'bg-white/[0.07] text-white border border-white/[0.08]' : 'text-[#7F8FA3] hover:text-slate-300 hover:bg-white/[0.03]'
              }`}
            >
              {cat}
            </button>
          ...
          <div className="space-y-1">
            {filtered.map((prompt) => (
              ...
              >
                ...
                <div className="text-[11px] text-[#7F8FA3] truncate mt-0.5">{prompt.content}</div>
              </button>
            ))}
            {filtered.length === 0 && (
              <div className="text-center py-8 text-[#7F8FA3] text-[13px]">No prompts found</div>
            )}
          </div>
        </div>
      </div>
