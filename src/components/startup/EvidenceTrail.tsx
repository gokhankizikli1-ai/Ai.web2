...
const ROLE_META: Record<string, { label: string; tone: string }> = {
  ...
  complaint: { label: 'Extracted complaint', tone: 'text-[#A78BFA] border-[#8B5CF6]/35 bg-[#8B5CF6]/[0.1]' },
  broad: { label: 'Broad evidence', tone: 'text-[#B6BBC6] border-white/[0.08] bg-white/[0.03]' },
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 space-y-3">
        {ordered.map((source) => (
          <div key={source}>
            <span className="block text-[10px] font-medium text-[#858B99] uppercase tracking-wider mb-1.5">
              {sourceLabel(source)}
            </span>
                ...
                return (
                  <li key={c.url} className="flex items-center gap-2 min-w-0">
                    <a
                      ...
                      className="flex items-center gap-1.5 min-w-0 text-[12px] text-slate-300 hover:text-[#A78BFA] transition-colors"
                    >
                      <ExternalLink className="h-2.5 w-2.5 shrink-0 text-[#858B99]" />
                      <span className="font-medium shrink-0">{domainOf(c.url)}</span>
                      <span className="text-[#858B99] truncate">{c.title}</span>
                    </a>
