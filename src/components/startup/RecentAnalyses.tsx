  ...
  return (
    ...
    >
      <summary className="flex items-center gap-2 cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden">
        ...
        <Clock3 className="h-3.5 w-3.5 text-[#A9B7C6]" />
        <span className="text-[13px] font-semibold text-slate-100">Recent analyses</span>
        <span className="text-[11px] text-[#7F8FA3]">({history.length})</span>
        <button
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); onClearHistory(); }}
          className="ml-auto flex items-center gap-1 text-[10px] text-[#7F8FA3] hover:text-[#C98A93] transition-colors"
        >
      ...
      <div className="space-y-1 mt-2.5">
        {history.map((entry) => (
          ...
          >
            <Target className="h-3 w-3 text-[#7F8FA3] group-hover/item:text-[#8FB4CC] shrink-0 transition-colors" />
            ...
            <span className="shrink-0 text-[11px] font-semibold text-slate-100">
              ...
              <span className="text-[9px] font-normal text-[#7F8FA3]">/100</span>
            </span>
            <span className={`shrink-0 text-[10px] ${CONFIDENCE_TONE[entry.report.summary.confidence] || 'text-[#A9B7C6]'}`}>
              {entry.report.summary.confidence}
            </span>
            <span className="shrink-0 text-[10px] text-[#7F8FA3] hidden sm:inline">{timeAgo(entry.savedAt)}</span>
          </button>
        ))}
      </div>
      <p className="text-[10px] text-[#7F8FA3] mt-2">Stored locally in this browser — restoring does not re-fetch data.</p>
    </details>
  );
}
