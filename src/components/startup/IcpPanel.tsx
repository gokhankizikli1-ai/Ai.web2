  ...
  return (
    <div className="rounded-xl border border-white/[0.05] bg-white/[0.01] p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        {icp?.isHypothesis && (
          <span className="px-1.5 py-0.5 rounded text-[9px] bg-[#8B5CF6]/[0.12] border border-[#8B5CF6]/35 text-[#A78BFA]">
            hypothesis
          </span>
        )}
      </div>
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={row.label} className="flex flex-col sm:flex-row sm:items-start gap-0.5 sm:gap-3">
              <span className="shrink-0 sm:w-32 text-[10px] text-[#858B99] sm:pt-0.5">{row.label}</span>
