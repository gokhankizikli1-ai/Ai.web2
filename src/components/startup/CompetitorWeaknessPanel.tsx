  ...
  return (
    <div className="rounded-xl border border-white/[0.05] bg-white/[0.01] p-4">
      <div className="flex items-center gap-2 mb-3">
        <Swords className="h-3.5 w-3.5 text-[#B6BBC6]" />
        <h3 className="text-[13px] font-semibold text-slate-100">Competitor weaknesses found in evidence</h3>
      </div>

      {competitors.length === 0 ? (
        <p className="text-[12px] text-[#B6BBC6]">No strong competitor mentions found in this run.</p>
      ) : (
        <div className="space-y-1.5">
              ...
              >
                ...
                {w ? (
                  <>
                    ...
                    <span className="text-[10px] text-[#858B99] shrink-0">
                      {w.evidence_count} evidence item{w.evidence_count === 1 ? '' : 's'}
                    </span>
                  </>
                ) : (
                  <span className="text-[11px] text-[#B6BBC6] min-w-0 flex-1">
          ...
          <p className="text-[10px] text-[#858B99] mt-1">
