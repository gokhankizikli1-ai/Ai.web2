...
function painTone(score: number): string {
  ...
  if (score >= 40) return 'text-[#A78BFA] border-[#8B5CF6]/40 bg-[#8B5CF6]/[0.12]';
...
export default function ComplaintClusterCard({
  ...
  return (
    <motion.div
      ...
      className="rounded-xl border border-[#272A33] bg-[#111217] p-4 transition-colors hover:border-[rgba(139, 92, 246,0.30)] hover:shadow-[0_0_0_1px_rgba(139, 92, 246,0.10)]"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          <span className="text-[11px] text-[#858B99] shrink-0">#{rank + 1}</span>
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-[10px] text-[#B6BBC6]">
      ...
      {quote && (
        <div className="mt-2.5 flex items-start gap-2 rounded-lg bg-[#161820] border border-[#272A33] px-2.5 py-2">
          <Quote className="h-3 w-3 text-[#8B5CF6] shrink-0 mt-0.5" />
          <p className="text-[12px] text-slate-300 leading-relaxed break-words min-w-0">
            “{quote.text}”
            <span className="ml-1.5 text-[9px] text-[#858B99]">— {sourceLabel(quote.source)}</span>
          </p>
        </div>
      )}
          ...
          {subScores.map((s) => (
            <div key={s.label}>
              <div className="h-1 rounded-full bg-white/[0.06] overflow-hidden">
                <div
                  className={`h-full rounded-full ${s.label === 'Saturation' ? 'bg-[#F87171]/70' : 'bg-[#8B5CF6]/60'}`}
                  style={{ width: `${Math.min(100, Math.max(0, s.value))}%` }}
                />
              </div>
              <span className="block mt-1 text-[9px] text-[#858B99]">{s.label}</span>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-wrap gap-1.5 mt-2.5">
            {cluster.evidence_urls.slice(0, 4).map((url) => (
              <a
                ...
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] text-[#B6BBC6] border border-white/[0.06] hover:text-[#A78BFA] hover:border-[#8B5CF6]/35 transition-colors max-w-[220px]"
              >
