    ...
    <div ref={rootRef} id="top" data-palette-family={ds.paletteFamily} data-premium={premium ? 'true' : undefined} onClick={handlePreviewLinkClick} className="text-slate-200 antialiased" style={{ ...rootStyle, scrollBehavior: 'smooth' }}>
      ...
      {activePage === LEAD_CAPTURE_PAGE ? (
        ...
        <div key={current.id} className="relative">
          ...
          {heroMotionLayer && <MotionAmbientLayer pattern={heroMotionLayer.pattern} intensity={heroMotionIntensity} reduce={Boolean(reduce)} className="opacity-70" />}
          {globalMotionLayer && <MotionAmbientLayer pattern={globalMotionLayer.pattern} intensity={globalMotionLayer.intensity} reduce={Boolean(reduce)} className="opacity-60" />}
