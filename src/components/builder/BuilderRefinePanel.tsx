    ...
    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.015] overflow-hidden">
      ...
      <div className="flex items-center gap-1.5 px-3 py-2.5 border-b border-white/[0.05] flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <button
            onClick={() => setOpen((v) => !v)}
            className="ml-0.5 flex h-7 w-7 items-center justify-center rounded-lg text-[#7F8FA3] hover:text-slate-300 hover:bg-white/[0.04] transition-colors"
            ...
            <div className="p-4 space-y-4">
              ...
              {tab === 'refine' ? (
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3.5 py-3 rounded-xl bg-black/20 border text-[13px] text-slate-200 placeholder:text-[#7F8FA3] focus:outline-none resize-none transition-colors"
                  style={{ borderColor: `${accent}30` }}
                />
              ) : (
  ...
  return (
    <button
      ...
      className={`px-3 py-1.5 rounded-md text-[11px] font-medium transition-colors ${
        active ? 'bg-white/[0.06] text-white' : 'text-[#7F8FA3] hover:text-slate-300'
      }`}
    >
      {label}
    </button>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      <p className="text-[11px] text-[#7F8FA3]">{label}</p>
      {children}
    </div>
  );
}
        ...
        return (
          <button
            ...
            style={isActive
              ...
              : { background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(255,255,255,0.07)', color: '#A9B7C6' }}
          >
            {opt}
          </button>
        );
