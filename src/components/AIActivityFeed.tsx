  ...
  if (status === 'active') {
    return (
      <span className="relative flex h-2 w-2">
        <span className="absolute inline-flex h-full w-full rounded-full bg-[#3B82F6]/50 animate-ping" style={{ animationDuration: '2s' }} />
        <span className="relative inline-flex rounded-full h-2 w-2 bg-[#3B82F6]" />
      </span>
    );
  }
  if (status === 'completed') return <CheckCircle2 className="h-3 w-3 text-[#4ADE80]/60" />;
  return <Clock className="h-3 w-3 text-[#94A3B8]" />;
}

function CategoryIcon({ message }: { message: string }) {
  const lower = message.toLowerCase();
  if (lower.includes('code') || lower.includes('debug')) return <Code2 className="h-3 w-3 text-[#3B82F6]/50" />;
  if (lower.includes('research') || lower.includes('analysis')) return <Search className="h-3 w-3 text-[#3B82F6]/50" />;
  if (lower.includes('write') || lower.includes('draft')) return <FileText className="h-3 w-3 text-[#3B82F6]/50" />;
  if (lower.includes('market') || lower.includes('trade')) return <TrendingUp className="h-3 w-3 text-[#3B82F6]/50" />;
  return <Brain className="h-3 w-3 text-[#3B82F6]/50" />;
}
      ...
      >
        <div className="flex items-center gap-2.5">
          <div className="relative flex items-center justify-center">
            <Activity className="h-3 w-3 text-[#3B82F6]/50" />
            {activeCount > 0 && (
              <motion.span
                className="absolute -top-0.5 -right-0.5 h-[5px] w-[5px] rounded-full bg-[#3B82F6]"
                animate={{ scale: [1, 1.4, 1], opacity: [1, 0.6, 1] }}
                transition={{ duration: 2, repeat: Infinity }}
              />
            )}
          </div>
          <span className="text-[10px] text-[#94A3B8] font-semibold uppercase tracking-wider">Activity</span>
          {activeCount > 0 && (
            <span className="text-[10px] text-[#3B82F6]/70 font-medium flex items-center gap-1">
              <span className="relative flex h-1.5 w-1.5">
                <span className="absolute inline-flex h-full w-full rounded-full bg-[#3B82F6]/40 animate-ping" style={{ animationDuration: '2s' }} />
                ...
                <div key={activity.id} className="flex items-center gap-2.5 py-[2px]">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-[#94A3B8] truncate">{activity.message}</span>
                      {activity.progress !== undefined && activity.status === 'active' && (
                        <span className="text-[9px] text-[#3B82F6]/50 shrink-0 font-mono">{activity.progress}%</span>
                      )}
                    </div>
                    {activity.detail && (
                      <span className="text-[9px] text-[#94A3B8] block truncate">{activity.detail}</span>
                    )}
                  </div>
