        ...
        >
          ...
          >
            ...
            <div className="relative px-8 pt-8 pb-6 border-b border-white/[0.04]">
              <button
                onClick={onClose}
                className="absolute top-4 right-4 h-8 w-8 flex items-center justify-center rounded-lg text-[#858B99] hover:text-white hover:bg-white/[0.04] transition-all"
              >
                <X className="h-4 w-4" />
              </button>

              <div className="flex items-center gap-2 mb-2">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#8B5CF6]/10 border border-[#8B5CF6]/15">
                  <Sparkles className="h-4 w-4 text-[#8B5CF6]" />
                </div>
                <span className="text-[11px] font-semibold text-[#8B5CF6]/70 uppercase tracking-wider">Upgrade</span>
              </div>
              <h2 className="text-2xl font-semibold text-white mb-1 tracking-tight">Unlock the full power of KorvixAI</h2>
              <p className="text-[13px] text-[#858B99]">Choose the plan that fits your ambition. Upgrade anytime.</p>
              ...
              <div className="flex items-center gap-3 mt-5">
                <div className="flex items-center rounded-lg bg-white/[0.03] border border-white/[0.05] p-0.5">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-[11px] font-medium transition-all ${!yearly ? 'bg-white/[0.06] text-white' : 'text-[#858B99] hover:text-[#B6BBC6]'}`}
                  >
                    Monthly
                  </button>
                  <button
                ...
                {yearly && (
                  <motion.span
                    ...
                    className="text-[11px] text-[#4ADE80]/70 font-medium flex items-center gap-1"
                  >
              ...
              {PLANS.map((plan, i) => (
                <motion.div
                  ...
                  className={`relative rounded-xl border ${plan.border} bg-gradient-to-b ${plan.gradient} p-5 flex flex-col ${
                    plan.popular ? 'ring-1 ring-[#8B5CF6]/10' : ''
                  }`}
                >
                  {plan.popular && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                      <div className="flex items-center gap-1 rounded-full bg-[#8B5CF6]/15 border border-[#8B5CF6]/20 px-2.5 py-0.5 text-[10px] font-semibold text-[#8B5CF6]">
                  ...
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#8B5CF6]/10 border border-[#8B5CF6]/15">
                        <plan.icon className="h-3.5 w-3.5 text-[#8B5CF6]" />
                      </div>
                    ...
                    <div className="flex items-baseline gap-1">
                      {plan.price.monthly > 0 ? (
                        <>
                          ...
                          <span className="text-[11px] text-[#858B99]">/month</span>
                        </>
                      ) : (
                        <span className="text-2xl font-bold text-white">Custom</span>
                      )}
                    </div>
                    <p className="text-[11px] text-[#858B99] mt-1.5">{plan.description}</p>
                  </div>

                  <ul className="space-y-2 mb-5 flex-1">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-start gap-2 text-[11px] text-[#B6BBC6]">
                        <Check className="h-3 w-3 shrink-0 mt-0.5 text-[#8B5CF6]/60" />
                        {f}
                      </li>
                    ))}
                  </ul>
            ...
            <div className="px-8 py-4 border-t border-white/[0.03] flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-[11px] text-[#858B99]">
                <Shield className="h-3 w-3" />
                Secure payment. Cancel anytime.
              </div>
              <span className="text-[10px] text-[#B6BBC6]">30-day money-back guarantee</span>
            </div>
          </motion.div>
        </motion.div>
      )}
