...
function TypeIcon({ mime }: { mime: string }) {
  ...
  if (m.startsWith('image/')) return <ImageIcon className="h-3 w-3 text-[#3B82F6]/70" />;
  if (m.startsWith('video/')) return <FileVideo className="h-3 w-3 text-[#3B82F6]/70" />;
  if (m === 'application/pdf' || m.startsWith('text/'))
    return <FileText className="h-3 w-3 text-[#3B82F6]/70" />;
  return <Paperclip className="h-3 w-3 text-[#CBD5E1]/70" />;
}
...
export default function AssetChip({ asset, onDismiss, compact = false }: AssetChipProps) {
  ...
  const borderClass = (() => {
    ...
    if (status === 'uploading') return 'border-[#3B82F6]/25';
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col min-w-0 flex-1">
        ...
        <span className={`${compact ? 'text-[9px]' : 'text-[10px]'} text-[#94A3B8] leading-tight`}>
      ...
      <div className="shrink-0 flex items-center justify-center">
        {status === 'uploading' && (
          <Loader2 className="h-3 w-3 text-[#3B82F6]/70 animate-spin" />
        )}
        {status === 'ready' && (
      ...
      {onDismiss && (
        <button
          ...
          className="shrink-0 h-4 w-4 flex items-center justify-center rounded-full text-[#94A3B8] hover:text-slate-300 hover:bg-white/[0.06] transition-colors"
