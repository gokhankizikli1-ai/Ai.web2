/**
 * ProjectConnectorCards — the PER-PROJECT connector surfaces.
 *
 * These are the provider-specific cards: connection status for one Korvix
 * project, plus each provider's own resource picker (GitHub account +
 * repository, Vercel project, Slack channels). Every id they send came from a
 * server-provided list and is re-verified live by the backend before it can be
 * bound — that validation is the reason this code is REUSED rather than
 * reimplemented anywhere else.
 *
 * WHERE THEY RENDER NOW
 * ---------------------
 * They are no longer the Connectors landing page. That page is now an
 * ACCOUNT-level grid ("connected accounts and tools for my Korvix account"), and
 * these cards render inside a provider's Manage drawer, scoped to whichever
 * project the user is configuring. So a Slack channel picker can never make a
 * card several hundred pixels tall on the landing page again — it lives in a
 * focused management surface, on demand.
 *
 * The cards themselves are unchanged in behaviour: same endpoints, same
 * server-side revalidation, same truthful error reporting.
 */
...
export type Notify = (message: string, type: 'success' | 'error' | 'info') => void;
