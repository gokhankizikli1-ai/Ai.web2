...
const CONFIG: Record<WorkspaceType, { headline: string; subline: string; gradient: string; actions: Action[] }> = {
  startup: {
    ...
    gradient: 'from-[#3B82F6]/[0.03] to-[#60A5FA]/[0.02]',
    actions: [
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="mb-6"
      >
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#3B82F6]/[0.06] border border-[#3B82F6]/25">
      ...
      <p className="text-[13px] text-[#94A3B8] text-center max-w-md mb-8">{config.subline}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full max-w-xl">
        {config.actions.map((action, i) => (
          <motion.button
            ...
            whileHover={{ y: -2, borderColor: 'rgba(59, 130, 246,0.40)' }}
            whileTap={{ scale: 0.97 }}
            onClick={() => navigate(action.path)}
            className="flex flex-col items-center gap-2.5 p-4 rounded-xl border border-[#3B82F6]/25 bg-white/[0.01] hover:bg-[#3B82F6]/[0.06] transition-colors text-center"
          >
            <action.icon className="w-5 h-5 text-[#3B82F6]" />
            <span className="text-[12px] font-medium text-[#CBD5E1]">{action.label}</span>
            <ArrowRight className="w-3 h-3 text-[#3B82F6]/70" />
          </motion.button>
        ))}
      </div>
    </motion.div>
