...
function ScoreBar({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-[9px] text-[#94A3B8] w-16 shrink-0">{label}</span>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        {badge && (
          <span className="px-1.5 py-0.5 rounded text-[8px] font-medium bg-white/[0.04] text-[#94A3B8] border border-white/[0.04]">{badge}</span>
        )}
      </div>
      <h3 className="text-[12px] font-semibold text-white/90 mb-0.5">{title}</h3>
      <p className="text-[10px] text-[#94A3B8] leading-relaxed">{desc}</p>
      <div className="flex items-center gap-1 mt-2 text-[9px] text-[#60A5FA]/50 group-hover:text-[#60A5FA]/70 transition-colors">
        <span>Open</span>
...
function BackButton({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex items-center gap-1 text-[11px] text-[#94A3B8] hover:text-white/60 transition-colors mb-3">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {[
          { label: 'Products Researched', value: '12', icon: Package, color: 'from-[#60A5FA]/20 to-[#60A5FA]/20' },
          { label: 'Hooks Generated', value: '48', icon: Sparkles, color: 'from-[#60A5FA]/20 to-[#60A5FA]/20' },
          { label: 'Ad Sets Created', value: '6', icon: Megaphone, color: 'from-[#FACC15]/20 to-[#FACC15]/20' },
          { label: 'Store Health', value: '--', icon: BarChart3, color: 'from-[#4ADE80]/20 to-[#60A5FA]/20' },
        ].map((s) => (
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          <ToolCard icon={Search} title="Product Research" desc="Find winning products with AI-powered trend analysis, saturation scores, and margin estimates." color="bg-gradient-to-br from-[#60A5FA] to-[#60A5FA]" onClick={() => onTool('product-research')} />
          <ToolCard icon={Layout} title="AI Store Builder" desc="Generate complete Shopify store sections — hero, product grid, reviews, FAQ, and trust badges." color="bg-gradient-to-br from-[#60A5FA] to-[#60A5FA]" onClick={() => onTool('store-builder')} />
          <ToolCard icon={ShoppingCart} title="Shopify Connect" desc="Connect your store, sync products, and manage everything from one dashboard." color="bg-gradient-to-br from-[#4ADE80] to-[#60A5FA]" onClick={() => onTool('shopify')} badge="Coming soon" />
          <ToolCard icon={Sparkles} title="TikTok Hooks" desc="Generate viral hooks, UGC scripts, and short-form video structures for any product." color="bg-gradient-to-br from-[#60A5FA] to-[#F87171]" onClick={() => onTool('tiktok')} />
          ...
          <ToolCard icon={DollarSign} title="Pricing Optimizer" desc="Calculate optimal pricing, margins, break-even ROAS, and upsell suggestions." color="bg-gradient-to-br from-[#60A5FA] to-[#60A5FA]" onClick={() => onTool('pricing')} />
          ...
          <ToolCard icon={Target} title="Product Page Analyzer" desc="Analyze your product pages for conversion optimization and get AI recommendations." color="bg-gradient-to-br from-[#60A5FA] to-[#60A5FA]" onClick={() => onTool('product-research')} />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="p-4 rounded-xl border border-white/[0.04] bg-white/[0.01] space-y-3">
          {[
            { icon: Search, name: 'Product Research Agent', desc: 'Finds trending products with winning potential', color: 'from-[#60A5FA] to-[#60A5FA]', status: 'active' },
            { icon: Layout, name: 'Store Builder Agent', desc: 'Creates optimized store pages and sections', color: 'from-[#60A5FA] to-[#60A5FA]', status: 'active' },
            { icon: Sparkles, name: 'Ads Creator Agent', desc: 'Generates TikTok hooks and Meta ad campaigns', color: 'from-[#60A5FA] to-[#F87171]', status: 'standby' },
            { icon: DollarSign, name: 'Pricing Agent', desc: 'Optimizes margins, pricing, and upsell strategy', color: 'from-[#60A5FA] to-[#60A5FA]', status: 'standby' },
          ].map((agent, i) => (
            <div key={agent.name} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-medium ${agent.status === 'active' ? 'bg-[#4ADE80]/[0.08] text-[#4ADE80]' : 'bg-slate-500/[0.06] text-[#CBD5E1]'}`}>
    ...
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search products..."
            className="w-full h-8 pl-9 pr-3 rounded-lg bg-white/[0.02] border border-white/[0.04] text-[12px] text-white placeholder:text-[#94A3B8] outline-none focus:border-white/[0.08]" />
        </div>
          ...
          {filtered.map((product) => (
            ...
            >
              <div className="flex items-start justify-between mb-2.5">
                <div className="flex items-center gap-2.5">
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/[0.03] border border-white/[0.05]">
                    <Package className="h-4 w-4 text-[#CBD5E1]" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                    ...
                    <p className="text-[10px] text-[#94A3B8]">{product.category} · {product.price}</p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-[8px] text-[#94A3B8]">Trend Score</p>
                </div>
              </div>

              <p className="text-[11px] text-[#CBD5E1] mb-2.5">{product.description}</p>
              ...
              <div className="space-y-1.5 mb-2.5">
                ...
                <ScoreBar label="Viral Pot." value={product.viral} color="bg-[#60A5FA]" />
                <ScoreBar label="Competitor" value={product.competitor} color="bg-[#F87171]" />
                <ScoreBar label="Audience" value={product.audience} color="bg-[#60A5FA]" />
              </div>
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <div className="pt-2.5 border-t border-white/[0.03] space-y-2">
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <div className="p-2 rounded-lg bg-[#60A5FA]/[0.04] border border-[#60A5FA]/10 text-center">
                          <p className="text-[9px] text-[#60A5FA]/60">Sell Price</p>
                          <p className="text-[13px] font-semibold text-[#60A5FA]">{product.price}</p>
                        </div>
                        <div className="p-2 rounded-lg bg-[#60A5FA]/[0.04] border border-[#60A5FA]/10 text-center">
                          <p className="text-[9px] text-[#60A5FA]/60">Viral Score</p>
                          <p className="text-[13px] font-semibold text-[#60A5FA]">{product.viral}/100</p>
                        </div>
                      </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-8">
          ...
          <p className="text-[12px] text-[#94A3B8]">No products found</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="space-y-2.5 mb-4">
        <div>
          <label className="text-[10px] text-[#94A3B8] mb-1 block">Product Name</label>
          <input value={product} onChange={e => setProduct(e.target.value)} placeholder="e.g. posture corrector"
            className="w-full h-8 px-3 rounded-lg bg-white/[0.02] border border-white/[0.04] text-[12px] text-white placeholder:text-[#94A3B8] outline-none focus:border-white/[0.08]" />
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="text-[10px] text-[#94A3B8] mb-1 block">Target Audience</label>
            <input value={audience} onChange={e => setAudience(e.target.value)} placeholder="e.g. office workers"
              className="w-full h-8 px-3 rounded-lg bg-white/[0.02] border border-white/[0.04] text-[12px] text-white placeholder:text-[#94A3B8] outline-none focus:border-white/[0.08]" />
          </div>
          <div>
            <label className="text-[10px] text-[#94A3B8] mb-1 block">Pain Point</label>
            <input value={painPoint} onChange={e => setPainPoint(e.target.value)} placeholder="e.g. back pain"
              className="w-full h-8 px-3 rounded-lg bg-white/[0.02] border border-white/[0.04] text-[12px] text-white placeholder:text-[#94A3B8] outline-none focus:border-white/[0.08]" />
          </div>
        </div>
        <div>
          <label className="text-[10px] text-[#94A3B8] mb-1 block">Tone</label>
          <div className="flex gap-1 flex-wrap">
            {tones.map(t => (
              <button key={t} onClick={() => setTone(t)}
                className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-all ${tone === t ? 'bg-[#60A5FA]/[0.12] text-[#60A5FA] border border-[#60A5FA]/20' : 'text-[#94A3B8] border border-white/[0.04] hover:text-[#CBD5E1]'}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        <button
          ...
          style={{ background: 'linear-gradient(135deg, #60A5FA, #60A5FA)', boxShadow: '0 4px 16px rgba(59, 130, 246,0.15)' }}>
      ...
      <AnimatePresence>
        {output && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            {/* 3-Second Hooks */}
            <div className="p-3.5 rounded-xl border border-[#60A5FA]/10 bg-[#60A5FA]/[0.03]">
              <h4 className="text-[11px] font-semibold text-[#60A5FA]/80 mb-2 flex items-center gap-1.5">
              ...
              {output.hook3s.map((h, i) => (
                <div key={i} className="flex items-start gap-2 py-1.5 border-b border-white/[0.02] last:border-0">
                  <span className="text-[9px] text-[#60A5FA]/40 mt-0.5 shrink-0">{i + 1}</span>
            ...
            <div className="p-3.5 rounded-xl border border-white/[0.04] bg-white/[0.015]">
              <h4 className="text-[11px] font-semibold text-white/70 mb-2 flex items-center gap-1.5">
                <Smartphone className="h-3 w-3 text-[#60A5FA]" /> UGC Script
              </h4>
              <pre className="text-[10px] text-[#CBD5E1] leading-relaxed whitespace-pre-wrap">{output.ugcScript}</pre>
            </div>

            {/* Problem/Solution */}
            <div className="p-3.5 rounded-xl border border-white/[0.04] bg-white/[0.015]">
              <h4 className="text-[11px] font-semibold text-white/70 mb-2 flex items-center gap-1.5">
                <Target className="h-3 w-3 text-[#60A5FA]" /> Problem / Solution Angle
              </h4>
              <pre className="text-[10px] text-[#CBD5E1] leading-relaxed whitespace-pre-wrap">{output.problemSolution}</pre>
            </div>
            ...
            <div className="p-3.5 rounded-xl border border-white/[0.04] bg-white/[0.015]">
              ...
              {output.ctas.map((c, i) => (
                <p key={i} className="text-[10px] text-[#CBD5E1] py-0.5">{i + 1}. {c}</p>
              ))}
            </div>
            ...
            <div className="p-3.5 rounded-xl border border-white/[0.04] bg-white/[0.015]">
              ...
              {output.structure.map((s, i) => (
                <p key={i} className="text-[10px] text-[#CBD5E1] py-0.5">{s}</p>
              ))}
            </div>
          </motion.div>
        )}
    ...
    <div>
      ...
      <div className="space-y-2.5 mb-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            <label className="text-[10px] text-[#94A3B8] mb-1 block">Key Benefit</label>
        ...
        {output && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            {[
              ...
              { title: 'Headlines', items: output.headlines, icon: Type as React.ElementType, color: 'text-[#60A5FA]', border: 'border-[#60A5FA]/10', bg: 'bg-[#60A5FA]/[0.03]' },
              { title: 'Descriptions', items: output.descriptions, icon: FileText as React.ElementType, color: 'text-[#60A5FA]', border: 'border-[#60A5FA]/10', bg: 'bg-[#60A5FA]/[0.03]' },
            ].map((section) => (
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="p-2.5 rounded-lg bg-white/[0.015] border border-white/[0.04]">
                <p className="text-[9px] text-[#94A3B8] mb-1">CTA Buttons</p>
                {output.ctas.map((c, i) => <p key={i} className="text-[10px] text-[#4ADE80]/80 py-0.5">{c}</p>)}
              </div>
              <div className="p-2.5 rounded-lg bg-white/[0.015] border border-white/[0.04]">
                <p className="text-[9px] text-[#94A3B8] mb-1">Audiences</p>
                {output.audiences.map((a, i) => <p key={i} className="text-[10px] text-[#60A5FA]/80 py-0.5">{a}</p>)}
              </div>
              <div className="p-2.5 rounded-lg bg-white/[0.015] border border-white/[0.04]">
                <p className="text-[9px] text-[#94A3B8] mb-1">Creative Formats</p>
                {output.creatives.map((c, i) => <p key={i} className="text-[10px] text-[#60A5FA]/80 py-0.5">{c}</p>)}
              </div>
            </div>
          </motion.div>
        )}
    ...
    <div>
      ...
      <div className="space-y-2.5 mb-4">
        <textarea
          ...
          rows={3}
          className="w-full px-3 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.04] text-[12px] text-white placeholder:text-[#94A3B8] outline-none focus:border-white/[0.08] resize-none"
        />
        <button
          onClick={generate}
          disabled={!prompt.trim() || generating}
          className="w-full h-9 rounded-xl text-[12px] font-semibold text-white transition-all disabled:opacity-30 hover:brightness-110"
          style={{ background: 'linear-gradient(135deg, #60A5FA, #3B82F6)', boxShadow: '0 4px 16px rgba(59, 130, 246,0.15)' }}>
      ...
      <AnimatePresence>
        {generated && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="p-3.5 rounded-xl border border-white/[0.04] bg-white/[0.015]">
              <h4 className="text-[11px] font-semibold text-white/70 mb-2 flex items-center gap-1.5">
                <Palette className="h-3 w-3 text-[#60A5FA]" /> Color Palette
              </h4>
            ...
            <div className="p-3.5 rounded-xl border border-white/[0.04] bg-white/[0.015]">
              <h4 className="text-[11px] font-semibold text-white/70 mb-2 flex items-center gap-1.5">
                <Type className="h-3 w-3 text-[#60A5FA]" /> Typography
              </h4>
              ...
              <div className="space-y-1.5">
                {sections.map((s, i) => (
                  <div key={s.name} className="flex items-center gap-2.5 py-2 border-b border-white/[0.02] last:border-0">
                    <div className="flex h-6 w-6 items-center justify-center rounded-md bg-[#60A5FA]/[0.08] text-[9px] text-[#60A5FA] font-semibold shrink-0">{i + 1}</div>
                    <div>
            ...
            >
              ...
              Export to Shopify
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1 rounded-lg bg-[#151C28] border border-white/[0.08] text-[9px] text-[#CBD5E1] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center text-center py-10">
        ...
        <p className="text-[11px] text-[#94A3B8] max-w-xs mb-6">Connect your Shopify store to sync products, track analytics, and manage everything from KorvixAI.</p>
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={f.label} className="p-3 rounded-xl border border-white/[0.02] bg-white/[0.01] opacity-40">
            <f.icon className="h-4 w-4 text-[#94A3B8] mb-1.5" />
            <p className="text-[11px] font-medium text-white/50">{f.label}</p>
            <p className="text-[9px] text-[#94A3B8]">{f.desc}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <div>
          <label className="text-[10px] text-[#94A3B8] mb-1 block">Product Cost ($)</label>
        ...
        <div>
          <label className="text-[10px] text-[#94A3B8] mb-1 block">Shipping Cost ($)</label>
          <input type="number" value={shipping} onChange={e => setShipping(e.target.value)}
            className="w-full h-8 px-3 rounded-lg bg-white/[0.02] border border-white/[0.04] text-[12px] text-white outline-none focus:border-white/[0.08]" />
        </div>
        <div>
          <label className="text-[10px] text-[#94A3B8] mb-1 block">Target Margin (%)</label>
          <input type="number" value={targetMargin} onChange={e => setTargetMargin(e.target.value)}
            className="w-full h-8 px-3 rounded-lg bg-white/[0.02] border border-white/[0.04] text-[12px] text-white outline-none focus:border-white/[0.08]" />
        </div>
        <div>
          <label className="text-[10px] text-[#94A3B8] mb-1 block">Competitor Price ($)</label>
      ...
      {/* Results */}
      <div className="grid grid-cols-2 gap-2">
        <div className="p-3 rounded-xl border border-[#60A5FA]/10 bg-[#60A5FA]/[0.04]">
          <p className="text-[9px] text-[#60A5FA]/60 mb-1">Suggested Price</p>
          <p className="text-[18px] font-semibold text-[#60A5FA]">${result.suggested}</p>
        </div>
        ...
        <div className="p-3 rounded-xl border border-[#60A5FA]/10 bg-[#60A5FA]/[0.04]">
          <p className="text-[9px] text-[#60A5FA]/60 mb-1">Profit / Order</p>
          <p className="text-[18px] font-semibold text-[#60A5FA]">${result.profit}</p>
        </div>
      ...
      {/* Upsell */}
      <div className="mt-3 p-3 rounded-xl border border-[#60A5FA]/10 bg-[#60A5FA]/[0.04]">
        <div className="flex items-center gap-2">
          <Zap className="h-4 w-4 text-[#60A5FA]" />
          <div>
            <p className="text-[10px] text-[#60A5FA]/60">Upsell Bundle Suggestion</p>
            <p className="text-[13px] font-semibold text-[#60A5FA]">${result.upsell} bundle price (+35% AOV)</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center text-center py-8 mb-4">
        ...
        <p className="text-[11px] text-[#94A3B8] max-w-xs">Link your store to see live revenue, orders, conversion rates, and more.</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {[
          ...
          <div key={m.label} className="p-3 rounded-xl border border-white/[0.02] bg-white/[0.01] opacity-40">
            <p className="text-[9px] text-[#94A3B8] mb-1">{m.label}</p>
            <p className="text-[16px] font-semibold text-white/50 tabular-nums">{m.value}</p>
            <p className="text-[8px] text-[#94A3B8]">{m.sub}</p>
          </div>
        ))}
      </div>
    </div>
