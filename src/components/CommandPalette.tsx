...
function PaletteIcon({ id, selected }: { id: string; selected: boolean }) {
  ...
  if (!icon) return <Terminal className="h-4 w-4 text-[#7F8FA3]" />;
  return (
    <span className={selected ? 'text-[#7EA6BF]' : 'text-[#7F8FA3]'}>
      {icon}
    </span>
  );
}
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-white/[0.04]">
          <Search className="h-4.5 w-4.5 text-[#7F8FA3]" />
          <input
            ref={inputRef}
            type="text"
            value={query}
            ...
            className="flex-1 bg-transparent text-[15px] text-white placeholder:text-[#7F8FA3] outline-none"
            autoFocus
          />
          <kbd className="hidden sm:inline-flex items-center rounded-md bg-white/[0.04] border border-white/[0.06] px-2 py-0.5 text-[10px] text-[#7F8FA3] font-mono">
            ESC
          </kbd>
        </div>
        ...
        <div className="max-h-[380px] overflow-y-auto scrollbar-thin py-2">
          {flatItems.length === 0 && (
            <div className="flex flex-col items-center py-10 text-center">
              <Search className="h-8 w-8 text-[#A9B7C6] mb-3" />
              <p className="text-[13px] text-[#7F8FA3] mb-1">No commands found</p>
              <p className="text-[11px] text-[#A9B7C6]">Try a different search term</p>
            </div>
          )}
            ...
            return (
              <div key={category} className="mb-1">
                <div className="px-5 py-1.5 flex items-center gap-2">
                  <span className="text-[10px] font-semibold text-[#7F8FA3] uppercase tracking-wider">{category}</span>
                  ...
                  return (
                    <motion.button
                      ...
                      className={`w-full flex items-center gap-3 px-5 py-2.5 text-left transition-colors duration-100 ${
                        isSelected ? 'text-white' : 'text-[#A9B7C6] hover:text-slate-300'
                      }`}
                    >
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-colors ${
                        isSelected ? 'bg-[#7EA6BF]/10' : 'bg-white/[0.02]'
                      }`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {item.shortcut && (
                          <div className="text-[11px] text-[#7F8FA3]">{item.shortcut}</div>
                        )}
                      </div>
                      {isSelected && (
                        <motion.div
                          ...
                          className="flex items-center gap-1 text-[#7F8FA3]"
                        >
        ...
        <div className="flex items-center justify-between px-5 py-2.5 border-t border-white/[0.04] bg-white/[0.01]">
          <div className="flex items-center gap-2 text-[10px] text-[#7F8FA3]">
