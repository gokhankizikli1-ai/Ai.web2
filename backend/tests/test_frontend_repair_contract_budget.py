# coding: utf-8
"""Regression: contract-aware quality-repair budget + reasoning (Phase 2).

Production runs `VITE_WEB_BUILD_QUALITY_REPAIR_MODE = all_delta`, so quality-repair emits the bounded
`frontend-delta-v1` upsert set — yet the backend used to size EVERY `[FRONTEND REPAIR REQUEST]` as a
full-source frontend task (30,000 max_output_tokens + medium reasoning). This proves the canonical
server-side resolvers now:

  * size a quality-repair whose EXPLICIT response contract is frontend-delta-v1 at 16,000, while a
    full re-emit (frontend-files-v1) and any missing/unknown signal stay 30,000 (conservative);
  * use LOW reasoning for a quality-repair (both contracts) while leaving initial-generation,
    contract-repair, revision and reviews UNCHANGED.

`_frontend_repair_contract` / `frontend_task_max_output_tokens` / `_frontend_reasoning_effort` /
`_frontend_task_kind` are pure functions; `ai_client` imports provider SDKs at load, so skip cleanly
in a minimal sandbox rather than fail collection.
"""
...
def test_E_initial_generation_remains_30k_regardless_of_contract():
    assert frontend_task_max_output_tokens("initial-generation", _REGISTERED) == 30_000
    # even if a (nonsensical) delta contract were passed, only quality-repair is contract-scoped.
    assert frontend_task_max_output_tokens("initial-generation", _REGISTERED, "frontend-delta-v1") == 30_000
...
def test_G_initial_generation_reasoning_unchanged_medium():
    assert _frontend_reasoning_effort(_generation_msg()) == "medium"
...
def test_contract_repair_and_revision_reasoning_unchanged_medium():
    # These are NOT quality-repair; their reasoning must remain medium (unchanged).
