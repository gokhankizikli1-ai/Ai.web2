...
#: with five fresh connectors, ≈126 when two refreshes are claimed). It catches
#: a new read being added silently; the no-growth assertions below are what
#: prove the shape is flat.
#:
...
def test_a_stale_page_open_writes_only_the_refresh_lease(env, client, calls,
                                                         counting_sqlite):
    _fill(env, 5)
    _bind(env, "github", last_sync="")
    counting_sqlite["count"] = 0
    ...
    writes = [s for s in counting_sqlite["statements"]
              if s.lower().startswith(("insert", "update", "delete"))]
    assert all("connector_refresh_attempts" in w for w in writes), writes
    assert calls == ["github"]
