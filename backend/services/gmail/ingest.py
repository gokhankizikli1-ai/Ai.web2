# coding: utf-8
"""Gmail connector — the single bridge from normalized message → Business Brain.

Mirrors `github.ingest`: this module does exactly ONE thing — call the canonical
...
connect time), never from message content — so an email can never scope an
observation to another user's project.
"""
...
def ingest_one(connection: GmailConnection, normalized: Dict[str, Any]) -> str:
    ...
    try:
        res = obs.record_observation(
            ...
            source=normalized.get("source") or "gmail",
        ...
        logger.warning("gmail.ingest.record failed: %s", type(exc).__name__)
        return "rejected"
    if not res.get("ok"):
        return "rejected"
...
def ingest_many(connection: GmailConnection,
