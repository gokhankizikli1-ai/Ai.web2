...
_WEBSITE_BUILDER_PROMPT = (
    ...
    "PIPELINE — run two upstream agents IN ORDER before generating, then build:\n"
    "  1) RESEARCH AGENT: interpret the idea; determine category language, audience\n"
    "     expectations, conversion patterns, the trust signals this category needs,\n"
    "     common visual patterns, and the RISKS TO AVOID. If you have live research\n"
    "     tools, use them and cite real URLs only; otherwise reason from knowledge\n"
    "     and label it strategy inference (never fabricate sources).\n"
    "  2) UI / ART DIRECTOR AGENT: turn that research + brief into a SPECIFIC, dynamic\n"
    "     visual direction — visual mood, typography, a full color system, layout\n"
    "     feeling, a concept-specific visual metaphor, motion and density. Never a\n"
    "     default indigo/cyan skin; different ideas must get different art direction.\n"
    "     Express it in the ## Design Direction fields.\n"
    "  3) BUILD: generate sections/copy/code and DO NOT ignore the two artifacts —\n"
    "     use the Art Direction for visuals and the Research Agent for trust signals,\n"
    "     category language, conversion patterns and risks to avoid.\n"
    "  MANDATORY: the two agents are preparation, not the deliverable. NEVER stop after\n"
    "  the Design Direction — every build MUST output ALL six sections below, especially\n"
    "  ## Page Sections, ## Generated Copy and ## Frontend Code. A reply that ends after\n"
    "  the agent/design analysis is an INCOMPLETE build and is not acceptable.\n\n"
