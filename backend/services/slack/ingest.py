# coding: utf-8
"""Slack connector — the single bridge from normalized message → Business Brain.

Mirrors `vercel.ingest` / `gmail.ingest` / `github.ingest` / `google_calendar.ingest`:
this module does exactly ONE thing — call the canonical
`observations_store.record_observation()` seam with the connection's
authoritative (user_id, project_id) scope. It NEVER creates a task, decision,
plan, or run; NEVER calls a model or forces an embedding; NEVER touches memory /
approvals / workflows.

The user_id is taken from the CONNECTION (derived from the project's owner at
install time), never from message content — so a message body, an author id, or
a channel name can never scope an observation to another user's project.
"""
...
def ingest_one(connection: SlackConnection, normalized: Dict[str, Any]) -> str:
    ...
    try:
        res = obs.record_observation(
            ...
            source=normalized.get("source") or "slack",
        ...
        logger.warning("slack.ingest.record failed: %s", type(exc).__name__)
        return "rejected"
    if not res.get("ok"):
        return "rejected"
...
def ingest_many(connection: SlackConnection,
