...
# Terminal statuses — a stale worker must not resurrect one of these to
# `running`. (Retry deliberately re-queues, which is allowed.)
...
def insert(record: JobRecord) -> JobRecord:
    ...
    try:
        ...
        # Idempotency collision — normalize to the backend-agnostic error so
        # the manager's dedup path works identically on SQLite and Postgres.
        raise JobIdempotencyConflict(str(exc)) from exc
    except Exception as e:
...
def update(record_id: str, **fields) -> Optional[JobRecord]:
    ...
    # Stale-worker guard: never resurrect a TERMINAL job to `running`. A
    # duplicate/stale worker trying to mark an already-succeeded/failed/
    # cancelled job as running is a no-op. Retry (→queued) is unaffected.
    where = "id=?"
    ...
    try:
        with _conn() as c:
            c.execute(f"UPDATE jobs SET {', '.join(sets)} WHERE {where}", params)
        _bump("updates")
        return get(record_id)
    except Exception as e:
...
def bump_max_attempts(record_id: str, new_max: int) -> bool:
    """Raise a job's max_attempts cap (retry path). Kept out of the `update`
    whitelist because the cap is creation-time policy; this is the sole
    sanctioned mutator so no caller reaches into the connection directly."""
    _ensure_init()
    if not record_id:
        return False
    try:
        ...
        logger.warning("jobs.store.bump_max_attempts id=%s error: %s", record_id, e)
        return False
