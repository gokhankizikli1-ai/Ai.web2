# coding: utf-8
"""Google Calendar connector — OAuth state store (CSRF + ownership binding).

Same shape as `gmail.state_store` / `vercel.setup_state`, in its OWN table —
this is not a parallel auth system, it is the per-connector row of the one
established pattern (each connector owns its states so a Gmail callback can
never consume a Calendar state, or vice versa).
...
It must therefore be:
  ...
  * OWNERSHIP-BOUND — the (user_id, project_id, provider) that INITIATED the
                   ...
                   honoured — an attacker cannot attach a Calendar to another
                   user's project.
...
CREATE INDEX IF NOT EXISTS ix_calendar_states_expiry ON calendar_oauth_states(expires_at);
"""
...
def create_state(*, owner_user_id: str, project_id: str,
    ...
    init_calendar_state_table()
    token = secrets.token_urlsafe(32)  # 256 bits
    now = _now()
    expires = now + timedelta(seconds=cal_config.state_ttl_s())
    with _sqlite.connection(DB_PATH) as c:
        c.execute(
            """INSERT INTO calendar_oauth_states
               (state, owner_user_id, project_id, provider, created_at, expires_at, consumed_at)
               VALUES (?, ?, ?, ?, ?, ?, NULL)""",
            (token, owner_user_id, project_id, str(provider or PROVIDER_CALENDAR),
...
def consume(state: str) -> Optional[OAuthState]:
    ...
    try:
        with _sqlite.writer_tx(DB_PATH) as c:
            row = c.execute(
                "SELECT * FROM calendar_oauth_states WHERE state=?", (state,),
            ).fetchone()
            if row is None:
                return None
            d = dict(row)
            ...
            if exp is None or exp < now:
                return None  # expired
            cur = c.execute(
                "UPDATE calendar_oauth_states SET consumed_at=? WHERE state=? AND consumed_at IS NULL",
                (_iso(now), state),
            )
            ...
            return OAuthState(
                ...
                provider=str(d.get("provider") or PROVIDER_CALENDAR),
            )
    except Exception as exc:
        logger.warning("calendar.state_store.consume failed: %s", type(exc).__name__)
        return None
...
def purge_expired(*, now: Optional[datetime] = None) -> int:
    ...
    try:
        with _sqlite.connection(DB_PATH) as c:
            cur = c.execute("DELETE FROM calendar_oauth_states WHERE expires_at < ?", (cutoff,))
...
def _reset_for_tests() -> None:
    try:
        with _sqlite.connection(DB_PATH) as c:
            c.execute("DROP TABLE IF EXISTS calendar_oauth_states")
    except Exception:
        pass


__all__ = [
    "OAuthState", "PROVIDER_CALENDAR", "init_calendar_state_table",
