...
async def costs_analytics(
    ...
    """Aggregate cost analytics (task #7). Owner-only."""
    _audit(user, "admin.costs.analytics.view", request)
    return envelope_ok(tracker.analytics(user_id=user_id))
...
async def costs_builds(
    ...
    return envelope_ok({"builds": builds, "count": len(builds)})
...
async def costs_build_detail(
    ...
    return envelope_ok(tracker.get_build(build_id))
...
async def costs_dashboard(
    ...
    """Self-contained HTML cost dashboard (task #7). Owner-only."""
    _audit(user, "admin.costs.dashboard.view", request)
    a = tracker.analytics()
    builds = tracker.list_builds(limit=50)
    return HTMLResponse(_render_dashboard(a, builds))
