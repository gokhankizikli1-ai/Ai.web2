...
_PROJECT_AUTHORITY_HEADER = (
    ...
    "NOT invent a subject, a domain, a customer or an activity for this "
    "project.\n"
    "\n"
    "EVIDENCE DISCIPLINE (how the answer must be built)\n"
    "The block carries an \u201cEvidence base\u201d section listing what this "
    "project's evidence establishes and what it does NOT. That list is binding, "
    "and these rules govern every sentence you write about this project:\n"
    "  1. OBSERVED FACTS AS FACTS. Report what a source recorded, in the "
    "source's own terms, with the source named: \u201cVercel reports the "
    "production deployment succeeded.\u201d\n"
    "  2. INTERPRETATION SEPARATELY, AND LABELLED. Any reading you add is "
    "yours, not the evidence's, and must be marked as a reading — and paired "
    "with its limit: \u201cThis confirms the deployment succeeded. It does not "
    "by itself show the application works or that anyone has used it.\u201d\n"
    "  3. AN EVENT PROVES ITSELF AND NOTHING MORE. A deployment proves a "
    "deployment. A merged pull request proves a merge. A passing check proves "
    "that check passed. Never promote one into functionality, testing, "
    "readiness, quality, user activity, customer feedback, goal progress or a "
    "business result — those require a source that recorded THAT, and the "
    "Evidence base says whether one exists.\n"
    "  4. ONE SOURCE IS NOT CORROBORATION. When a single tool is behind a "
    "statement, say so in the same sentence.\n"
    "  5. NAME WHAT IS MISSING, CONCRETELY. When the evidence does not answer "
    "the question, do not fill the gap with a generic project sentence "
    "(\u201cprogress is being made\u201d, \u201cfeedback is being "
    "evaluated\u201d, \u201cthe team is iterating\u201d). Say what is absent "
    "and what would settle it — a CI result, a person's report, a recorded "
    "goal.\n"
    ...
    async def event_stream() -> AsyncIterator[str]:
        ...
        if tool_capabilities:
            cap_note = (
                ...
                "CURRENT PRICE / MARKET / STOCK / CRYPTO / FX / COMMODITY POLICY "
                "(NON-NEGOTIABLE):\n"
                "  • NEVER answer a current price, quote, market cap, or exchange "
                "rate from memory or training data — those numbers are stale and "
                "must come ONLY from a fetched live block above.\n"
                "  • When you have a live block, cite the value with its "
                "timestamp/source if present.\n"
                "  • If NO live price data was fetched for this turn, say exactly: "
                "\"Live price data is unavailable right now, so I can't give an "
                "exact current price.\" (TR: \"Canlı fiyat verisi şu anda "
                "kullanılamıyor, bu yüzden tam güncel fiyatı veremem.\") — then "
                "offer to retry. Do NOT invent or estimate a number.\n"
                "  • If a ticker/symbol looks like a typo (e.g. 'ncda' → 'NVDA'), "
                "use the obvious correction, or ask 'NVDA/NVIDIA hissesini mi "
