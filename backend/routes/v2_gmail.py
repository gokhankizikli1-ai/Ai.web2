...
def disconnect_project(
    ...
    """Revoke the grant at Google (best-effort) and delete all local credential
    material. Ownership-enforced."""
    _ensure_enabled()
    _require_owned_project(project_id, user)
    conn = gm_store.get_connection(project_id)
    if conn is None:
    ...
    # Best-effort remote revoke using the stored refresh token, then delete the
    # row regardless of the remote outcome (disconnect must always succeed
    # locally). A missing encryption key just skips the remote revoke.
    try:
        rt = gm_store.decrypt_refresh_token(conn)
        if rt:
            gm_oauth.revoke_token(rt)
